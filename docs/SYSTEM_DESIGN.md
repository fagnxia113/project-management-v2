...
**v2.9 更新内容：**
- 设备入库表单新增字段：生产厂家、技术参数、校准证书到期时间、备注（仪器类）；生产厂家、技术参数、备注（假负载和线材类）
- 移除设备入库表单中的"仪器出厂编号"字段
- 设备台账页面新增列显示：生产厂家、技术参数、采购日期、校准到期时间、证书编号、颁发机构、附件描述、备注
- 设备台账页面支持横向滚动，方便查看所有列
- 设备详情页面新增字段显示：生产厂家、技术参数（可编辑）
- 设备详情页面移除"仪器出厂编号"字段
- 修复设备售出表单提交时的400错误，添加"售出原因"字段和验证
- 删除设备借用功能模块（包括前端页面、后端服务和路由、数据库迁移文件）
- 从导航菜单中移除"设备借用"菜单项
- 从系统路由中移除设备借用相关路由
...
- **报废/售出流程（不可逆操作）**:
  - **申请报废/售出**: 无法维修/无使用价值的设备可发起报废申请，有残值的设备可发起售出申请
  - **位置管理员审批**: 位置管理员审批报废/售出申请，通过后进入确认处理环节
  - **确认处理**: 审批通过后，确认报废/售出操作，设备状态变更为"已报废"（scrapped）
    - 仪器类：将设备location_status设置为'scrapped'，health_status设置为'scrapped'，从可用列表隐藏（仅管理员可查）
    - 负载/线缆类：从原设备库存中扣减报废/售出数量，库存台账直接清零，不再参与流转
  - **售出专用信息**: 售出类型需要记录售出价格和购买方信息
  - **状态变更规则**:
    - 处理时：location_status 变为 scrapped，health_status 变为 scrapped
  - **工作流集成**: 报废/售出流程与工作流引擎深度集成，确认处理节点通过专门的API调用更新设备状态，确保状态变更的可靠性
  - **不可逆性**: 报废/售出是不可逆操作，需要二次确认，所有记录保留，支持溯源
...
# 报废/售出管理（不可逆操作）
POST   /api/equipment/scrap-sales              # 申请报废/售出
PUT    /api/equipment/scrap-sales/:id/approve  # 审批通过
PUT    /api/equipment/scrap-sales/:id/reject   # 审批驳回
PUT    /api/equipment/scrap-sales/:id/process  # 确认处理
GET    /api/equipment/scrap-sales              # 报废/售出记录
GET    /api/equipment/scrap-sales/:id          # 报废/售出详情
...
| v2.2 | 2026-02-23 | 修复审批人解析，支持 user/department_manager/project_manager 类型 |
| v2.3 | 2026-02-23 | 修复审批记录权限控制，用户只能看到自己的审批记录 |
| v2.4 | 2026-02-23 | 新增员工管理权限控制（仅管理员可修改删除） |
| v2.5 | 2026-02-23 | 新增用户密码重置功能 |
| v2.6 | 2026-02-23 | 新增流程定义中指定用户从人员表多选功能 |
| v2.7 | 2026-02-24 | 修复入职申请岗位数据与岗位管理数据一致性问题 |
| v2.7 | 2026-02-24 | 新增岗位字段级联下拉功能（根据部门筛选岗位） |
| v2.8 | 2026-02-28 | 新增设备维修管理功能，包括维修申请、发货、收货流程 |
| v2.9 | 2026-02-28 | 新增设备报废/售出管理功能，包括报废/售出申请、审批、处理流程 |
