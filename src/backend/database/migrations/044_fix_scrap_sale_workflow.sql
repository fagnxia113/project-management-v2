-- 修复设备报废/售出工作流定义
-- 问题1：审批人配置应该使用位置管理员而不是固定的equipment_manager角色
-- 问题2：避免设备状态重复更新

-- 删除旧的工作流定义
DELETE FROM workflow_definitions WHERE id = 'wf-equipment-scrap-sale-001';

-- 创建新的设备报废/售出审批流程
INSERT INTO workflow_definitions (id, `key`, name, version, category, entity_type, status, node_config, form_schema)
VALUES (
  'wf-equipment-scrap-sale-1',
  ...
  JSON_OBJECT(
    'nodes', JSON_ARRAY(
      ...
      JSON_OBJECT('id', 'location-manager', 'type', 'userTask', 'name', '位置管理员审批', 'config', JSON_OBJECT('approvalConfig', JSON_OBJECT('approvalType', 'single', 'approverSource', JSON_OBJECT('type', 'field', 'value', 'location_manager_id')))),
      JSON_OBJECT('id', 'process', 'type', 'userTask', 'name', '确认处理', 'config', JSON_OBJECT('approvalConfig', JSON_OBJECT('approvalType', 'single', 'approverSource', JSON_OBJECT('type', 'field', 'value', 'location_manager_id')))),
