...
const WorkflowFormLauncher: React.FC<WorkflowFormLauncherProps> = ({ 
  ...
  const loadData = async () => {
    try {
      ...
      if (workflowDef.form_template_id) {
        const templateResponse = await fetch(`${API_URL.BASE}/api/form-templates/templates/${workflowDef.form_template_id}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        })
  ...
  const loadUserMap = async () => {
    try {
      ...
      const users = data.success ? data.data : (Array.isArray(data.data) ? data.data : [])
  ...
  const loadDepartmentMap = async () => {
    try {
      ...
      if (!response.ok) return
      const data = await response.json()
      const departments = data.success ? data.data : (Array.isArray(data.data) ? data.data : [])
      const map: Record<string, string> = {}
  ...
  const loadWarehouseMap = async () => {
    try {
      ...
      if (!response.ok) return
      const data = await response.json()
      const warehouses = data.success ? data.data : (Array.isArray(data.data) ? data.data : [])
      const map: Record<string, string> = {}
  ...
  const loadProjectMap = async () => {
    try {
      ...
      if (!response.ok) return
      const data = await response.json()
      const projects = data.success ? data.data : (Array.isArray(data.data) ? data.data : [])
      const map: Record<string, string> = {}
  ...
  const loadPositionMap = async () => {
    try {
      ...
      if (!response.ok) return
      const data = await response.json()
      const positions = data.success ? data.data : (Array.isArray(data.data) ? data.data : [])
      const map: Record<string, string> = {}
